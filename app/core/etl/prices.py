...
from app.db.connection import get_connection, get_duckdb_connection
...
def load_prices_5m(symbols=None, period=None, conn=None) -> int:
    ...
    if cfg.postgres_url:
        # Use Postgres via SQLAlchemy engine
        try:
            ...
            print(f"Error loading prices to Postgres: {e}")
            return 0
    else:
        # Fallback to DuckDB
        def _do(c):
            c.execute(
                ...
                (symbol TEXT, datetime TIMESTAMP, open DOUBLE, high DOUBLE, low DOUBLE, 
                 close DOUBLE, volume DOUBLE, source_load_ts TIMESTAMP, load_batch_id TEXT, 
        ...
        if conn:
            return _do(conn)
        with get_duckdb_connection() as c:
            return _do(c)
